...
*/
    ...
    func updateScore(){
        if score > 200 {
//            viewController?.performSegueWithIdentifier("backToContainer", sender: viewController)
        }
    ...
    */
    ...
    //add shapes
    func addShape() {
        let randomColor = Int(arc4random_uniform(level)) + 1
        let shapeStyle = Int(Int(arc4random_uniform(3)))
        //print(rad)
        var shape: Shape!
        var shapeKind: ShapeType!
        switch shapeStyle{
        case 0:
            shapeKind = ShapeType.Circle
              shape = Shape(type: shapeKind, movement: arrayOfColors[randomColor - 1].circle, color: randomColor)
        case 1:
            shapeKind = ShapeType.Square
              shape = Shape(type: shapeKind, movement: arrayOfColors[randomColor - 1].square, color: randomColor)
        case 2:
            shapeKind = ShapeType.Triangle
              shape = Shape(type: shapeKind, movement: arrayOfColors[randomColor - 1
